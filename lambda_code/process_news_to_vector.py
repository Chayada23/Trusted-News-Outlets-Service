...
def fetch_reporters():
    ...
    while "LastEvaluatedKey" in response:
        response = REPORTER_TABLE.scan(ExclusiveStartKey=response["LastEvaluatedKey"])
        items.extend(response.get("Items", []))
    return items
...
def delete_scraping(incident_id):
    try:
        SCRAPING_TABLE.delete_item(Key={"incident_id": incident_id})
    except Exception as e:
        print(f"[WARN] delete_scraping failed: {e}")

def delete_reporter(incident_id):
    try:
        REPORTER_TABLE.delete_item(Key={"incident_id": incident_id})
    except Exception as e:
